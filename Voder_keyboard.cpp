...
int tick( void * outBuff, void * inBuff, unsigned int nBuffFrames,
  ...
  for ( unsigned int i=0; i < nBuffFrames; ++i ) {
    ...
    for ( unsigned int j = 0; j < 10; ++j ) {
      // Each filter
      if ( sg->button_states[i] == 0 ) {
	// filter off
      } else if ( sg->button_states[i] == 3 ) { // High filter
	if ( sg->mode == 0 ) {
	  //val += sg->hiss->tick();
	  val += sg->keys[i]->tick( hval );
	} else {
	  //val += sg->buzz->tick();
	  val += sg->keys[i]->tick( bval );
	}
      }
    }
...
void set_up( SoundGens & sg ) {
  ...
  filter[0]->setStates(  225, 0.0, 1.0 ); // 000 - 225
  filter[1]->setStates(  450, 0.0, 1.0 ); // 255 - 450
  filter[2]->setStates(  700, 0.0, 1.0 ); // 450 - 700
  filter[3]->setStates( 1000, 0.0, 1.0 ); // 700 - 1000
  filter[4]->setStates( 1400, 0.0, 1.0 ); // 1000 - 1400
  filter[5]->setStates( 2000, 0.0, 1.0 ); // 1400 - 2000
  filter[6]->setStates( 2700, 0.0, 1.0 ); // 2000 - 2700
  filter[7]->setStates( 3800, 0.0, 1.0 ); // 2700 - 3800
  filter[8]->setStates( 5400, 0.0, 1.0 ); // 3800 - 5400
  filter[9]->setStates( 7500, 0.0, 1.0 ); // 5400 - 7500
